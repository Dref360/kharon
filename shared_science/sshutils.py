...
def get_ssh_keys(token: str) -> Tuple[str, str]:
    """Get ssh keys for this token.

    Returns:
        Private and Public key file.
    """
    hashed_key = hash_token(token)
    cachedir = pjoin(user_cache_dir("shared-science", "shared-science"), 'sshkey')
    os.makedirs(cachedir, exist_ok=True)
    private_key_file = pjoin(cachedir, f'{hashed_key}.key')
    public_key_file = pjoin(cachedir, f'{hashed_key}.key.pub')
